...
module RubyLang
  ...
  module Config
    ...
    def self.included(base)
      ...
      YAML.load(File.open('_config.yml'))['rubylang'].each_pair do |k,v|
  ...
  class Admin < Thor
    ...
    def watch
      ...
      in_root { run "trap 'kill $jekyllPid $compassPid' Exit; jekyll --auto & jekyllPid=$!; compass watch source & compassPid=$!; wait" }
    end

    desc 'preview', "preview the site in a web browser"
    def preview
      say "## Launching webserver to preview your site"
      in_root { run "trap 'kill $jekyllPid $compassPid' Exit; jekyll --auto --server & jekyllPid=$!; compass watch source & compassPid=$!; wait" }
    end
